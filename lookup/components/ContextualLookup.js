        ...
        {activeTab === 'resources' && (
          <div className="space-y-3">
            <div className="space-y-2">
              <button className="flex items-center space-x-2 w-full text-left text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 p-2 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                </svg>
              ...
              <button className="flex items-center space-x-2 w-full text-left text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 p-2 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                ...
                <span>Configure Dictionaries</span>
              </button>
            </div>
                ...
                <div className="space-y-1">
                  {data.resources.map((resource, index) => (
                    <a
                      ...
                      className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 block p-1 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                    >
                      {resource.title}
                    </a>
                  ))}
        ...
        {activeTab === 'analyzer' && (
          <div className="space-y-3">
              ...
              <div className={`p-2 rounded-lg border ${
                ...
                <div className={`text-xs font-medium ${
                  ...
                  {data.aiStatus.status === 'online' ? (
                    <>
                      🤖 AI Online
                      {data.aiStatus.hasOpenAI && data.aiStatus.hasNVIDIA && (
                        <span className="ml-1">(OpenAI + NVIDIA)</span>
                      )}
                      {data.aiStatus.hasOpenAI && !data.aiStatus.hasNVIDIA && (
                        <span className="ml-1">(OpenAI)</span>
                      )}
                      {!data.aiStatus.hasOpenAI && data.aiStatus.hasNVIDIA && (
                        <span className="ml-1">(NVIDIA)</span>
                      )}
                    </>
                  ) : (
                    <>
                      ...
                      <div className="text-xs mt-1 opacity-75">
                        No API keys configured. Add NVIDIA_API_KEY or LOOKUP_OPENAI_API_KEY to enable AI features.
                      </div>
                    </>
                  )}
                </div>
            ...
            {mounted && data.aiStatus?.status === 'online' && (
              <div className="space-y-2">
                <button
                  onClick={() => analyzeSentence('summarize')}
                  disabled={isAnalyzingSentence || selectedText.length < 10}
                  className="w-full p-2 text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-800/50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  📝 Summarize
                </button>
                <button
