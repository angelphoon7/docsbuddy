...
let openai = null;
if (process.env.LOOKUP_OPENAI_API_KEY) {
  openai = new OpenAI({
    apiKey: process.env.LOOKUP_OPENAI_API_KEY,
  });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!openai || !process.env.LOOKUP_OPENAI_API_KEY) {
...
Current analysis type: ${analysisType}`;
    ...
    if (error.code === 'invalid_api_key') {
      return res.status(401).json({ 
        error: 'Invalid lookup API key. Please check your LOOKUP_OPENAI_API_KEY.' 
      });
    }
...
function getFallbackResponse(sentence, analysisType) {
  ...
  return {
    ...
    timestamp: new Date().toISOString(),
    isFallback: true,
    note: 'AI analysis not available. Using fallback response. Add LOOKUP_OPENAI_API_KEY to your .env.local for AI-powered analysis.'
  };
}
